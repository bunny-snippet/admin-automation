...
def _scope(request):
    ...
    office = next((name for name in offices if name.casefold() == requested.casefold()), offices[0])
...
def notifications_data(request):
    ...
    if HIDE_PERSONAL:
        hidden = ClientAccess.objects.filter(office_name__iexact="Personal")
        queryset = queryset.exclude(client__in=hidden).exclude(
            client__isnull=True, device_id__in=hidden.exclude(device_id="").values("device_id")
        )
    ...
    return {"ok": True, "notifications": rows, "unread_count": unread, "pagination": pagination, "notification_filter": mode,
            "scope_note": "Personal office is excluded." if HIDE_PERSONAL else "Includes Personal and unassigned devices on this server."}
