...
def panel_access_api(request: HttpRequest) -> JsonResponse:
    ...
    clients = ClientAccess.objects.exclude(office_name__iexact="Personal").select_related("config_bundle").prefetch_related(
        Prefetch(
            "allowed_ips",
            ...
            if action == "resize":
                ...
                with transaction.atomic():
                    for target in targets:
                        ...
                        if excess:
                            remove_ids = list(
                                available.order_by("-created_at", "-pk")
                ...
                return panel_json({
                    "ok": True,
                    "message": (
                        f"Resized {len(targets)} pool(s) for {scope_label} to {target_count}/{threshold}; "
                        f"removed {deleted} excess available rows and queued {queued} refill(s)."
                    ),
                })
...
def _permission_source(value: str) -> str:
    return {
        "global-default": "Default policy",
        "office": "Office policy",
        "device": "PC override",
...
def _product_row(client: ClientAccess, latest_version: str = "") -> dict[str, Any]:
    ...
    if not product:
        ...
        if parts:
            product = (
                ClientAccess.DESKTOP_PRODUCT_LEGACY
                if tuple(parts) >= (1, 7)
                else ClientAccess.DESKTOP_PRODUCT_OPTIX
            )
    ...
    return {
        "code": product,
        "label": {
            ClientAccess.DESKTOP_PRODUCT_OPTIX: "OPTIX",
            ClientAccess.DESKTOP_PRODUCT_LEGACY: "I am the best",
        }.get(product, "Not detected"),
        "version": version,
        "evidence": evidence,
...
def panel_optix_api(request: HttpRequest) -> JsonResponse:
    ...
    if request.method == "POST":
        try:
            ...
            if action == "save_office":
                ...
                with transaction.atomic():
                    policy.save()
                    changed = ClientAccess.objects.filter(office_name__iexact=office).update(
                        release_channel=release_channel,
                        activation_mode=activation_mode,
                    )
                return panel_json({"ok": True, "message": f"OPTIX defaults and release assignment saved for {changed} PC(s) in {office}."})
            ...
            if action in {"save_activation_security", "rotate_activation_key"}:
                ...
                if action == "save_activation_security":
                    ...
                    security.save()
                    return panel_json({"ok": True, "message": "Global OPTIX activation rule updated."})
                new_key = f"OPTIX-ACT-{secrets.token_urlsafe(32)}"
                ...
                return panel_json({
                    "ok": True,
                    "message": "Old activation key revoked. Copy the new key now.",
                    "activation_key": new_key,
                    "activation_revision": int(security.activation_revision),
                })
            ...
            if action == "save_device":
                ...
                return panel_json({"ok": True, "message": f"OPTIX access saved for {client.name}."})
    ...
    return panel_json({
        ...
        "office": office,
        "policy": {
            **_policy_row(policy, office),
            "release_channel": next(iter(release_channels)) if len(release_channels) == 1 else "mixed",
            "activation_mode": next(iter(activation_modes)) if len(activation_modes) == 1 else "mixed",
        },
        "security": {
            "required": bool(security.activation_required) if security else False,
            "configured": bool(security.activation_key_hash) if security else False,
...
def _release_row(item: DesktopRelease | DesktopComponentRelease, kind: str) -> dict[str, Any]:
    ...
    return {
        ...
        "size": int(item.artifact_size or 0),
...
def panel_releases_api(request: HttpRequest) -> JsonResponse:
    if request.method == "POST":
        ...
        try:
            body = _body(request)
            action = str(body.get("action") or "").strip().lower()
            kind = str(body.get("kind") or "component").strip().lower()
            ...
            with transaction.atomic():
                ...
                if action == "revoke":
                    ...
                    item.status = item.STATUS_REVOKED
            ...
            message = exc.message_dict if isinstance(exc, ValidationError) and hasattr(exc, "message_dict") else str(exc)
            return panel_json({"ok": False, "message": message}, status=400)

    office = _selected_office(request.GET.get("office"), _visible_offices())
    clients = list(
        ClientAccess.objects.filter(office_name__iexact=office)
        .order_by("system_number", "name", "pk")
        .values("id", "system_number", "name", "device_id")
    ) if office else []
    rows = [
        *[_release_row(item, "application") for item in DesktopRelease.objects.all()[:60]],
        *[_release_row(item, "component") for item in DesktopComponentRelease.objects.all()[:120]],
    ]
    ...
    return panel_json({
        ...
        "clients": clients,
        "rows": rows[:120],
        "waiting_count": sum(1 for row in rows if row["status"] == "draft"),
        "live_count": sum(1 for row in rows if row["status"] == "published"),
    })
