...
class StandaloneSchedulerAssetTests(SimpleTestCase):
    ...
    def test_cron_installer_preserves_unrelated_content_and_is_idempotent(self):
        import importlib.util
        root = Path(__file__).resolve().parent.parent
        spec = importlib.util.spec_from_file_location("catalog_cron_installer", root / "deploy" / "install_ys_catalog_cron.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        fragment = (root / "deploy" / "warrior-ys-catalog-sync.cron").read_text()
        current = "# Existing private jobs\nMAILTO=admin@example.invalid\n15 3 * * * existing-job --arg=fixture-secret\n"
        updated = module.replace_managed_block(current, fragment)
        self.assertTrue(updated.startswith(current))
        self.assertEqual(module.replace_managed_block(updated, fragment), updated)
        newer = fragment.replace("--cron-guard", "--cron-guard ")
        self.assertTrue(module.replace_managed_block(updated, newer).startswith(current))
        self.assertEqual(updated.count(module.BEGIN), 1)
        with self.assertRaises(ValueError):
            module.replace_managed_block(updated + module.BEGIN + "\n", fragment)
