...
def backfill_identity(apps, schema_editor):
    ...
    for client in ClientAccess.objects.filter(desktop_client_product="").iterator(chunk_size=100):
        audit = (
            BootstrapAudit.objects.filter(client_id=client.pk)
            .exclude(app_version="")
            .order_by("-id")
            .only("app_version", "created_at")
            .first()
        )
        if audit is None:
            continue
        parts = [int(value) for value in re.findall(r"\d+", audit.app_version)[:2]]
        product = "legacy" if parts and tuple(parts) >= (1, 7) else "optix"
...
class Migration(migrations.Migration):
    dependencies = [("control", "0038_desktop_client_identity")]
