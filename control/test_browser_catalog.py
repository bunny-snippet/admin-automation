...
class CatalogEndpointTests(TestCase):
    ...
    def optix_request(self):
        from django.core import signing
        from .views import TOKEN_SALT
        token = signing.dumps({"browser_catalog_product": "optix"}, salt=TOKEN_SALT)
        return self.factory.get("/api/v1/browser-catalog/", HTTP_AUTHORIZATION="Bearer " + token)
    ...
    @patch("control.views._authenticated_client", return_value=SimpleNamespace(desktop_client_product="legacy"))
    def test_endpoint_rejects_authenticated_legacy_client(self, authenticated):
        from .views import browser_catalog
    ...
    def test_endpoint_returns_503_until_validated_snapshot_exists(self, authenticated):
        from .views import browser_catalog
        self.assertEqual(browser_catalog(self.optix_request()).status_code, 503)
    ...
    def test_endpoint_delivers_private_discovery_only_metadata(self, authenticated):
        ...
        response = browser_catalog(self.optix_request())
...
@override_settings(
    TRUST_PROXY_HEADERS=False, TRUST_APP_REPORTED_IPV4=False,
    REQUIRE_REPORTED_IP_MATCH=True, BOOTSTRAP_RATE_LIMIT_PER_MINUTE=1000,
    LOCAL_TESTING_MODE=False,
)
class OptixCatalogDeliveryTests(TestCase):
    def setUp(self):
        from django.core.cache import cache
        from .models import ClientAccess, ConfigBundle
        cache.clear()
        bundle = ConfigBundle(name="Isolated catalog fixture", version=7)
        bundle.set_payload({"APP_API_KEY": "fixture-existing-key"})
        bundle.save()
        self.access = ClientAccess.objects.create(
            name="Catalog fixture", ipv4="203.0.113.10", device_id="catalog-fixture",
            office_name="Fixture office", system_number="9", config_bundle=bundle,
        )
        catalog.sync_resource("common", lambda: catalog.validate_common(common_data()))

    def bootstrap(self, product=None, version="1.6.1"):
        body = {"reported_ipv4": "203.0.113.10", "device_id": "catalog-fixture", "app_version": version}
        if product is not None:
            body["client_product"] = product
        return self.client.post("/api/v1/bootstrap/", data=json.dumps(body), content_type="application/json", REMOTE_ADDR="203.0.113.10")

    def get_catalog(self, token, *, remote="203.0.113.10", device="catalog-fixture"):
        return self.client.get("/api/v1/browser-catalog/", HTTP_AUTHORIZATION="Bearer " + token, HTTP_X_DEVICE_ID=device, REMOTE_ADDR=remote)

    def test_only_explicit_optix_bootstrap_advertises_descriptor(self):
        for product, version in (("legacy", "1.7.46"), (None, "1.6.1"), (None, "1.7.46"), ("dollar", "1.6.1")):
            with self.subTest(product=product, version=version):
                response = self.bootstrap(product, version)
                self.assertEqual(response.status_code, 200)
                self.assertNotIn("browser_catalog_sync", response.json())
        response = self.bootstrap("optix")
        self.assertEqual(response.status_code, 200)
        descriptor = response.json()["browser_catalog_sync"]
        self.assertEqual(descriptor["download_path"], "/api/v1/browser-catalog/")
        self.assertRegex(descriptor["revision"], r"^[0-9a-f]{64}$")
        self.assertNotIn("catalogs", descriptor)

    def test_catalog_uses_real_active_device_ip_bound_token(self):
        bootstrap = self.bootstrap("optix").json()
        token = bootstrap["access_token"]
        response = self.get_catalog(token)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["catalogs"], common_data())
        self.assertNotIn("fixture-existing-key", response.content.decode())
        self.assertEqual(self.get_catalog(token, remote="203.0.113.11").status_code, 403)
        self.assertEqual(self.get_catalog(token, device="other-fixture").status_code, 403)
        self.access.active = False
        self.access.save(update_fields=["active"])
        self.assertEqual(self.get_catalog(token).status_code, 403)

    def test_real_legacy_token_cannot_fetch_catalog(self):
        response = self.bootstrap("legacy", "1.7.46")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.get_catalog(response.json()["access_token"]).status_code, 403)

    def test_product_inference_cannot_authorize_catalog_or_upgrade_old_token(self):
        for product in (None, "dollar"):
            with self.subTest(product=product):
                response = self.bootstrap(product, "1.6.1")
                self.assertEqual(response.status_code, 200)
                token = response.json()["access_token"]
                self.access.refresh_from_db()
                self.assertEqual(self.access.desktop_client_product, "optix")
                self.assertEqual(self.get_catalog(token).status_code, 403)
                self.assertEqual(self.bootstrap("optix").status_code, 200)
                self.assertEqual(self.get_catalog(token).status_code, 403)

    def test_missing_snapshot_does_not_break_bootstrap_or_legacy_assignment(self):
        BrowserCatalogSnapshot.objects.all().delete()
        for product in ("optix", "legacy"):
            with self.subTest(product=product):
                response = self.bootstrap(product)
                self.assertEqual(response.status_code, 200)
                body = response.json()
                self.assertNotIn("browser_catalog_sync", body)
                self.assertEqual(body["tubelight_config"]["OFFICE_NAME"], "Fixture office")
                self.assertEqual(body["tubelight_config"]["SYSTEM_NUMBER"], "9")
                self.assertEqual(body["tubelight_config"]["APP_API_KEY"], "fixture-existing-key")
