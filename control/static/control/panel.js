(() => {
  ...
  const migrationStatus = (row, detailed = false) => {
    const phase = String(row.remote_action_phase || "queued").replaceAll("_", " ");
    const progress = Math.max(0, Math.min(100, Number(row.remote_action_progress || 0)));
    const failed = row.remote_action_phase === "failed" || Boolean(row.remote_action_error);
    const message = row.remote_action_error || row.remote_action_status_message || (phase === "queued" ? "Waiting for this PC to receive the command." : "Migration is in progress.");
    if (!detailed) return `<small class="${failed ? "danger-text" : "migration-inline"}">Migration · ${escapeHtml(phase)} · ${progress}%</small>`;
    return `<div class="migration-progress ${failed ? "is-failed" : ""}"><div class="migration-progress-head"><strong>${failed ? "Migration failed" : `Migration · ${escapeHtml(phase)}`}</strong><span>${progress}%</span></div><div class="migration-progress-track"><i style="width:${progress}%"></i></div><p>${escapeHtml(message)}</p><small>Last update: ${formatDate(row.remote_action_status_at)}</small></div>`;
  };
  ...
  function updateHeader() {
    const labels = {access:"Access", proxy:"Proxy", optix:"OPTIX Control", releases:"OPTIX Releases"};
  ...
  async function loadOptix() {
    ...
    const optixCount = data.rows.filter(row => row.product.code === "optix").length;
    const legacyCount = data.rows.filter(row => row.product.code === "legacy").length;
    const migrationAgentCount = data.rows.filter(row => row.product.code === "legacy" && row.migration_agent_ready).length;
    const publicCount = data.rows.filter(row => row.release_channel === "public").length;
    content.innerHTML = `<section class="workspace-head control-hero"><div><span class="eyebrow">Desktop fleet control</span><h2>OPTIX Control</h2><p>See what is actually running, assign releases and apply office or PC-level permissions.</p></div><div class="head-actions">${legacyCount ? `<button class="button button-secondary" id="office-migration" ${data.migration_ready && migrationAgentCount ? "" : "disabled"}>Migrate ready PCs (${migrationAgentCount})</button>` : ""}<button class="button button-secondary" id="activation-security">Activation security</button><button class="button button-primary" id="office-policy">Configure office</button></div></section>${officeTabs(data.offices, data.office, "optix")}<div class="fleet-metrics"><div class="fleet-metric"><span class="metric-icon is-optix">O</span><div><strong>${optixCount}</strong><small>OPTIX detected</small></div></div><div class="fleet-metric"><span class="metric-icon is-legacy">I</span><div><strong>${legacyCount}</strong><small>I am the best</small></div></div><div class="fleet-metric"><span class="metric-icon is-release">R</span><div><strong>${publicCount}/${data.rows.length}</strong><small>Public release</small></div></div><div class="fleet-metric"><span class="metric-icon is-security">A</span><div><strong>r${data.security.revision}</strong><small>${data.security.required ? "Activation enforced" : "Activation optional"}</small></div></div></div><div class="policy-strip"><div><span>Office providers</span><strong>${escapeHtml(data.policy.providers.join(", ") || "None")}</strong></div><div><span>Browsers</span><strong>${escapeHtml(data.policy.browsers.join(", ") || "None")}</strong></div><div><span>Devices</span><strong>${escapeHtml(data.policy.devices.join(", ") || "None")}</strong></div><div><span>Release assignment</span><strong>${escapeHtml(data.policy.release_channel === "mixed" ? "Mixed by PC" : data.policy.release_channel)}</strong></div></div><article class="card data-card fleet-table"><div class="table-toolbar"><div><strong>${escapeHtml(data.office || "No office")}</strong><span>Product is based on the latest successful desktop handshake—not on assigned permissions.</span></div><span class="security-chip">Key ${escapeHtml(data.security.hint || "not configured")}</span></div><div class="table-scroll"><table><thead><tr><th>System</th><th>Running app</th><th>Release</th><th>Permission source</th><th>Last seen</th><th>Access</th><th></th></tr></thead><tbody>${data.rows.length ? data.rows.map(row => `<tr><td><strong>${escapeHtml(row.system_number)}</strong><small>${escapeHtml(row.name)} · ${escapeHtml(row.bundle)}</small></td><td><span class="product-badge is-${escapeHtml(row.product.code || "unknown")}"><i></i>${escapeHtml(row.product.label)}</span><small>${row.product.version ? `v${escapeHtml(row.product.version)}` : "No handshake yet"}${row.product.activated ? ` · activated r${row.product.activation_revision}` : ""}</small></td><td><span class="release-badge is-${escapeHtml(row.release_channel)}">${escapeHtml(row.release_channel)}</span><small>${escapeHtml(row.activation_mode)}</small></td><td><span class="source-label">${escapeHtml(row.product.code === "legacy" ? "Legacy app" : row.permission_source)}</span>${row.remote_action ? (row.remote_action === "migrate_optix" ? migrationStatus(row) : `<small class="danger-text">Removal ${row.remote_acknowledged ? "acknowledged" : "pending"}</small>`) : ""}</td><td>${formatDate(row.last_seen)}</td><td>${statusPill(row.active, "Allowed", "Blocked")}</td><td class="align-right"><button class="link-button" data-optix-view="${row.id}">${row.product.code === "optix" ? "Control" : "Details"}</button></td></tr>`).join("") : emptyRows("No systems in this office.")}</tbody></table></div></article>`;
    ...
    document.querySelector("#office-migration")?.addEventListener("click", () => openMigrationConfirm());
  ...
  function openActivationSecurity() {
    const security = state.data.optix.security;
    openDrawer(`<div class="drawer-head"><div><span class="eyebrow">Global security</span><h2>Activation key</h2><p>Rotating creates a new key and immediately revokes the old revision for Public clients.</p></div><button class="dialog-close" data-drawer-close>Close</button></div><div class="drawer-body stack"><div class="security-overview"><span class="metric-icon is-security">A</span><div><span>Current key</span><strong>${escapeHtml(security.hint || "Not configured")}</strong><small>Revision ${security.revision}</small></div></div><form class="stack" id="activation-rule-form"><label class="toggle-control"><input type="checkbox" name="required" ${security.required ? "checked" : ""}><span>Require activation globally for Public channel</span></label><button class="button button-secondary" type="submit">Save activation rule</button></form><div class="destructive-zone"><span class="field-title">Revoke and rotate</span><p>The previous key stops working. The new value is shown only once in this panel.</p><button class="button button-danger" id="rotate-activation" type="button">Revoke old key &amp; generate new</button></div><div id="generated-key"></div></div>`);
    const ruleForm = drawer.querySelector("#activation-rule-form");
    ruleForm.addEventListener("submit", async event => { event.preventDefault(); try { const result=await postJson(endpoints.optix,{action:"save_activation_security",required:ruleForm.required.checked});toast(result.message);closeDrawer();await loadOptix(); } catch(error){toast(error.message,true);} });
    drawer.querySelector("#rotate-activation").addEventListener("click", async () => {
      if (!confirm("Revoke the current activation key and generate a new one?")) return;
      try { const result=await postJson(endpoints.optix,{action:"rotate_activation_key",required:ruleForm.required.checked}); const target=drawer.querySelector("#generated-key"); target.innerHTML=`<div class="generated-key"><span>Copy this key now</span><strong class="mono wrap">${escapeHtml(result.activation_key)}</strong><button class="button button-primary" type="button">Copy key</button></div>`; target.querySelector("button").onclick=()=>navigator.clipboard.writeText(result.activation_key).then(()=>toast("Activation key copied.")); toast(result.message); }
      catch(error){toast(error.message,true);}
    });
  }
  ...
  async function openOptixDevice(clientId) {
    try {
      ...
      const optixControls = row.product.code === "optix" ? `<label class="toggle-control"><input type="checkbox" name="override" ${row.override ? "checked" : ""}><span>Use individual permissions instead of office policy</span></label><div><span class="field-title">Providers</span>${checkGroup("providers",data.options.providers,chosen.providers)}</div><div><span class="field-title">Browsers</span>${checkGroup("browsers",data.options.browsers,chosen.browsers)}</div><div><span class="field-title">Profile devices</span>${checkGroup("devices",data.options.devices,chosen.devices)}</div><label class="toggle-control"><input type="checkbox" name="show_logs" ${chosen.show_logs ? "checked" : ""}><span>Show Logs tab</span></label>` : `<div class="legacy-notice"><span class="product-badge is-${escapeHtml(row.product.code || "unknown")}"><i></i>${escapeHtml(row.product.label)}</span><h3>OPTIX controls are not active on this PC</h3><p>${row.product.code === "legacy" ? `Latest handshake is from I am the best ${escapeHtml(row.product.version || "")}. Permissions will appear here after OPTIX connects.` : "No supported OPTIX handshake has been received yet."}</p></div><input type="hidden" name="override" value="${row.override ? "1" : "0"}">`;
      openDrawer(`<div class="drawer-head"><div><span class="eyebrow">${escapeHtml(row.office)} / System ${escapeHtml(row.system_number)}</span><h2>${escapeHtml(row.name)}</h2><p>${escapeHtml(row.bundle)}</p></div><button class="dialog-close" data-drawer-close>Close</button></div><form class="drawer-body stack" id="device-policy-form"><div class="device-identity"><span class="metric-icon ${row.product.code === "optix" ? "is-optix" : "is-legacy"}">${row.product.code === "optix" ? "O" : "I"}</span><div><strong>${escapeHtml(row.product.label)}</strong><small>${row.product.version ? `Version ${escapeHtml(row.product.version)}` : "Waiting for first handshake"}${row.product.activated ? ` · activation r${row.product.activation_revision}` : ""}</small></div></div><div class="section-panel"><span class="section-kicker">PC assignment</span><div class="two-fields"><label>Release channel<select name="release_channel"><option value="public" ${row.release_channel === "public" ? "selected" : ""}>Public</option><option value="testing" ${row.release_channel === "testing" ? "selected" : ""}>Testing</option></select></label><label>Activation rule<select name="activation_mode"><option value="inherit" ${row.activation_mode === "inherit" ? "selected" : ""}>Use global rule</option><option value="require" ${row.activation_mode === "require" ? "selected" : ""}>Require activation</option><option value="bypass" ${row.activation_mode === "bypass" ? "selected" : ""}>Bypass activation</option></select></label></div></div><label class="toggle-control"><input type="checkbox" name="active" ${row.active ? "checked" : ""}><span>Allow desktop access on this PC</span></label>${optixControls}<button class="button button-primary" type="submit">Save PC assignment</button>${row.product.code === "optix" ? `<div class="destructive-zone"><span class="field-title">Remote removal</span>${row.remote_action && !row.remote_action_acknowledged_at ? `<p>Removal command r${row.remote_action_revision} is waiting for this PC.</p><button class="button button-secondary" type="button" id="cancel-uninstall">Cancel pending removal</button>` : row.remote_action_acknowledged_at ? `<p>PC acknowledged the removal command at ${formatDate(row.remote_action_acknowledged_at)}.</p>` : `<p>Schedule a verified command that the supported OPTIX client acknowledges before uninstalling.</p><button class="button button-danger" type="button" id="schedule-uninstall">Schedule complete OPTIX removal</button>`}</div>` : row.product.code === "legacy" ? `<div class="destructive-zone"><span class="field-title">Migrate to OPTIX</span>${row.remote_action && !row.remote_action_acknowledged_at ? `${migrationStatus(row, true)}<button class="button button-secondary" type="button" id="cancel-remote-action" ${row.remote_action_phase && row.remote_action_phase !== "queued" && row.remote_action_phase !== "failed" ? "disabled" : ""}>Cancel pending migration</button>` : `<p>${row.migration_agent_ready ? "Ready: close the legacy app, remove electronBro and old OPTIX, then install and open the signed OPTIX release." : "First roll out I am the best v1.7.46 or newer to enable safe migration on this PC."}</p><button class="button button-primary" type="button" id="schedule-migration" ${data.migration_ready && row.migration_agent_ready ? "" : "disabled"}>Migrate this system</button>`}</div>` : ""}</form>`);
      if (row.product.code === "optix" && row.remote_action === "migrate_optix") {
        const zone = drawer.querySelector(".destructive-zone");
        if (zone) zone.innerHTML = `<span class="field-title">OPTIX migration</span>${migrationStatus(row, true)}`;
      }
      const form=drawer.querySelector("#device-policy-form");
      form.addEventListener("submit",async event=>{event.preventDefault();try{const result=await postJson(endpoints.optix,{action:"save_device",client_id:row.id,active:form.active.checked,release_channel:form.release_channel.value,activation_mode:form.activation_mode.value,override:row.product.code === "optix" ? form.override.checked : row.override,providers:row.product.code === "optix" ? selectedChecks(form,"providers") : row.providers,browsers:row.product.code === "optix" ? selectedChecks(form,"browsers") : row.browsers,devices:row.product.code === "optix" ? selectedChecks(form,"devices") : row.devices,show_logs:row.product.code === "optix" ? form.show_logs.checked : row.show_logs});toast(result.message);closeDrawer();await loadOptix();}catch(error){toast(error.message,true);}});
      drawer.querySelector("#schedule-uninstall")?.addEventListener("click",()=>openUninstallConfirm(row));
      drawer.querySelector("#schedule-migration")?.addEventListener("click",()=>openMigrationConfirm(row));
      ...
      drawer.querySelector("#cancel-remote-action")?.addEventListener("click",async()=>{try{const result=await postJson(endpoints.optix,{action:"cancel_remote_action",client_id:row.id});toast(result.message);closeDrawer();await loadOptix();}catch(error){toast(error.message,true);}});
    } catch(error){toast(error.message,true);}
  }

  function openUninstallConfirm(row){
    openDrawer(`<div class="drawer-head"><div><span class="eyebrow">High-risk action</span><h2>Remove OPTIX from this PC</h2><p>${escapeHtml(row.office)} / System ${escapeHtml(row.system_number)}</p></div><button class="dialog-close" data-drawer-close>Close</button></div><form class="drawer-body stack" id="uninstall-form"><div class="danger-panel"><strong>Remote uninstall</strong><span>The command is device-bound, revisioned and acknowledged before execution. Access remains a separate control.</span></div><label>Enter system number ${escapeHtml(row.system_number)} to confirm<input name="confirmation" autocomplete="off" required></label><button class="button button-danger" type="submit">Schedule removal</button></form>`);
  ...
  function openMigrationConfirm(row=null){
    const data=state.data.optix;
    const legacy=data.rows.filter(item=>item.product.code==="legacy");
    const target=row||legacy[0];
    if(!target){toast("No I am the best installation is available in this office.",true);return;}
    const scope=row?"system":"office";
    const expected=row?row.system_number:data.office;
    openDrawer(`<div class="drawer-head"><div><span class="eyebrow">Controlled migration</span><h2>${row?`Migrate system ${escapeHtml(row.system_number)}`:`Migrate ${escapeHtml(data.office)}`}</h2><p>Only detected I am the best installations are targeted.</p></div><button class="dialog-close" data-drawer-close>Close</button></div><form class="drawer-body stack" id="migration-form"><div class="danger-panel"><strong>Fresh OPTIX replacement</strong><span>I am the best closes first. electronBro and an existing OPTIX install are removed before the signed OPTIX installer runs.</span></div><label>Enter ${escapeHtml(expected)} to confirm<input name="confirmation" autocomplete="off" required></label><button class="button button-primary" type="submit">Schedule ${scope} migration</button></form>`);
    drawer.querySelector("#migration-form").addEventListener("submit",async event=>{event.preventDefault();try{const result=await postJson(endpoints.optix,{action:"schedule_migration",client_id:target.id,scope,confirmation:event.currentTarget.confirmation.value});toast(result.message);closeDrawer();await loadOptix();}catch(error){toast(error.message,true);}});
  }
  ...
  async function loadReleases() {
    const ticket = routeSequence;
    const data = await getJson(endpoints.releases,{office:state.office.releases});
    if (ticket !== routeSequence || state.route !== "releases") return;
    state.data.releases=data; state.office.releases=data.office;
    const latestDraft=data.rows.find(row=>row.status==="draft");
    content.innerHTML=`<section class="workspace-head control-hero"><div><span class="eyebrow">Signed desktop delivery</span><h2>OPTIX Releases</h2><p>Uploaded packages stay visible as waiting until you choose the channel and exact rollout scope.</p></div><div class="head-actions"><a class="button button-secondary" href="/admin/control/desktoprelease/add/">Upload installer</a><a class="button button-primary" href="/admin/control/desktopcomponentrelease/add/">Upload component</a></div></section>${officeTabs(data.offices,data.office,"releases")}<div class="release-overview"><div><span>Waiting for rollout</span><strong>${data.waiting_count}</strong><small>${latestDraft ? `${escapeHtml(latestDraft.component)} ${escapeHtml(latestDraft.version)} is latest` : "No uploaded drafts"}</small></div><div><span>Live rollouts</span><strong>${data.live_count}</strong><small>Published release records</small></div><div><span>Selected office</span><strong>${escapeHtml(data.office || "None")}</strong><small>${data.clients.length} targetable PC(s)</small></div></div><article class="card data-card release-table"><div class="table-toolbar"><div><strong>Release timeline</strong><span>Draft means uploaded to the VPS but not delivered to any app.</span></div></div><div class="table-scroll"><table><thead><tr><th>Package</th><th>Version</th><th>Channel</th><th>Scope</th><th>Uploaded</th><th>Status</th><th></th></tr></thead><tbody>${data.rows.length?data.rows.map(row=>{const scope=row.target_device_ids.length?`${row.target_device_ids.length} PC`:row.target_offices.length?row.target_offices.join(", "):"All assigned PCs";return `<tr><td><strong>${escapeHtml(row.component)}</strong><small>${escapeHtml(row.kind)} · ${escapeHtml(row.filename)} · ${fileSize(row.size)}</small></td><td><strong>${escapeHtml(row.version)}</strong><small>Build ${row.build_number} · ${escapeHtml(row.activation)}</small></td><td><span class="release-badge is-${escapeHtml(row.channel)}">${escapeHtml(row.channel)}</span></td><td>${escapeHtml(scope)}</td><td>${formatDate(row.created_at)}</td><td><span class="release-status is-${escapeHtml(row.status)}">${row.status==="draft"?"Waiting":escapeHtml(row.status)}</span></td><td class="align-right">${row.status==="draft"?`<button class="link-button" data-rollout="${row.kind}:${row.id}">Roll out</button>`:row.status==="published"?`<button class="link-button" data-rollout="${row.kind}:${row.id}">Add targets</button> <button class="link-button danger-text" data-revoke="${row.kind}:${row.id}">Revoke</button>`:""}</td></tr>`}).join(""):emptyRows("No release packages uploaded yet.")}</tbody></table></div></article>`;
    content.querySelectorAll("[data-rollout]").forEach(button=>button.addEventListener("click",()=>{const [kind,id]=button.dataset.rollout.split(":");openRollout(data.rows.find(row=>row.kind===kind&&String(row.id)===id));}));
    content.querySelectorAll("[data-revoke]").forEach(button=>button.addEventListener("click",async()=>{const [kind,id]=button.dataset.revoke.split(":");if(!confirm("Revoke this live rollout?"))return;try{const result=await postJson(endpoints.releases,{action:"revoke",kind,release_id:id});toast(result.message);await loadReleases();}catch(error){toast(error.message,true);}}));
  }

  function openRollout(row){
    const data=state.data.releases;
    openDrawer(`<div class="drawer-head"><div><span class="eyebrow">Release rollout</span><h2>${escapeHtml(row.component)} ${escapeHtml(row.version)}</h2><p>Build ${row.build_number} · ${escapeHtml(row.filename)}</p></div><button class="dialog-close" data-drawer-close>Close</button></div><form class="drawer-body stack" id="rollout-form"><div class="release-package"><span class="metric-icon is-release">R</span><div><strong>Uploaded and signed</strong><small>${fileSize(row.size)} · ${row.status==="published"?"live rollout; add another target":"waiting for rollout"}</small></div></div><label>Channel<select name="channel"><option value="testing" ${row.channel==="testing"?"selected":""}>Testing</option><option value="public" ${row.channel==="public"?"selected":""}>Public</option></select></label><div><span class="field-title">Rollout scope</span><div class="scope-cards"><label><input type="radio" name="scope" value="all"><span><strong>All assigned PCs</strong><small>Every device on the selected channel.</small></span></label><label><input type="radio" name="scope" value="office" checked><span><strong>${escapeHtml(data.office)}</strong><small>Only this office.</small></span></label><label><input type="radio" name="scope" value="device"><span><strong>One PC</strong><small>Select an individual Device ID below.</small></span></label></div></div><label>Individual PC<select name="client_id"><option value="">Choose a PC when using individual scope</option>${data.clients.map(client=>`<option value="${client.id}">System ${escapeHtml(client.system_number)} · ${escapeHtml(client.name)}</option>`).join("")}</select></label><div class="context-box"><span>Delivery rule</span><strong>The app receives this release only when its assigned channel and rollout target match.</strong></div><button class="button button-primary" type="submit">${row.status==="published"?"Add rollout target":"Publish rollout"}</button></form>`);
    const form=drawer.querySelector("#rollout-form");form.addEventListener("submit",async event=>{event.preventDefault();const scope=form.scope.value;if(scope==="device"&&!form.client_id.value){toast("Choose an individual PC.",true);return;}try{const result=await postJson(endpoints.releases,{action:"publish",kind:row.kind,release_id:row.id,channel:form.channel.value,scope,office:data.office,client_id:form.client_id.value});toast(result.message);closeDrawer();await loadReleases();}catch(error){toast(error.message,true);}});
  }
